  ...
  return (
    <>
      <ul className="course-tab nav nav-tabs justify-content-between">
        ...
        <li className="nav-item">
          <button
            className={`nav-link ${tab == "requirements" ? "active" : ""}`}
      ...
      <div className="tab-content">
        ...
        {!is_class && (
          <>{tab === "overview" && <CourseVideo courseSlug={courseSlug} />}</>
        )}
        {tab === "requirements" && <Requirements requirements={requirements} />}
        {tab === "witcf" && (
          <WhoIsThisCourseFor who_is_this_course_for={who_is_this_course_for} />
        )}
      </div>
    </>
  );
