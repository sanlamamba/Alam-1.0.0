...
const userSignin = async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      return res.status(404).json({ message: "Cet compte n'existe pas" });
    }

    if (!user.email_confirmed) {
      return res.status(404).json({
        message: "Votre email n'est pas verifié.",
      });
    }
    ...
    if (!user.status) {
      return res.status(404).json({
        message:
          "Cet compte est deactivé, veuillez contactez un administrateur",
      });
    }
